...
from monai_legacy_aspire.data import CacheDataset, load_decathalon_datalist, load_aspire_datalist
from monai_legacy_aspire.transforms import Randomizable, LoadNiftid, LoadPNGd, AddChanneld, Compose
from monai_legacy_aspire.apps.utils import download_and_extract
...
class AspireDataset(Randomizable, CacheDataset):
    """
    The Dataset to automatically download the data of Medical Segmentation Decathlon challenge
    (http://medicaldecathlon.com/) and generate items for training, validation or test.
    It's based on :py:class:`monai.data.CacheDataset` to accelerate the training process.
    ...
    def __init__(
        ...
        task: str,
        section: str,
        transform: Callable[..., Any] = LoadPNGd(["image", "label"]),
        seed: int = 0,
        val_frac: float = 0.2,
        cache_num: int = sys.maxsize,
        cache_rate: float = 1.0,
        ...
        if self.cross_validation:
            print("cv")
            data = self._generate_data_list_cv(dataset_dir)
    ...
    def _generate_data_list(self, dataset_dir):
        ...
        label_std = os.path.join(self.task,'dataset_' + str(self.gauss_std) + 'std.json' )
        datalist = load_aspire_datalist(os.path.join(dataset_dir, label_std), True, section, base_dir=dataset_dir)
        if section == "test":
            return datalist
        else:
            data = list()
    ...
    def _generate_data_list_cv(self, dataset_dir):
        # section = "training" if self.section in ["training", "validation"] else "test"
        section = self.section
        
        label_std = os.path.join(self.task,'fold' + str(self.fold) + '_' + str(self.gauss_std) + 'std.json' )
        datalist = load_aspire_datalist(os.path.join(dataset_dir, label_std), True, section, base_dir=dataset_dir)
